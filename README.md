...
- 🎯 **按问题类型输出结果** — 推演预测会先识别题型，再输出数值预测 / 反应推演 / 最佳匹配 / 判断结果，而不是所有问题都套同一种模板
- 💰 **成本可控** — 模型分层 + 选择性激活 + 批量异步，架构层面控制开销

## 推演预测（Forecast）

BeeClaw 的 Forecast 页面现在不是单纯的“多角色观点拼盘”，而是**按问题类型驱动主结果**：

- **ForecastValue（数值预测）**
  - 优先输出：预测值、预测区间、时间点、置信度、关键驱动因素
  - 适用问题：价格、人数、营收、汇率、概率区间等
- **Reaction（反应推演）**
  - 优先输出：反应顺序、主要分歧、关键反应
  - 适用问题：发布会后市场怎么反应、某事件如何发酵
- **BestMatch（最佳匹配）**
  - 优先输出：最匹配对象、为什么是它、备选为什么次优
  - 适用问题：哪个人群更适合、哪个方案最匹配
- **Judgment / Strategy / Evolution / Risk / Ranking / Insight**
  - 分别对应明确判断、行动策略、演变路径、风险评估、排序比较、关键洞察

### 当前 Forecast 设计原则

- **问题类型决定主结果结构**
- **场景类型只影响推演过程，不直接决定 UI 卡片**
- **主结果先回答问题，推演视角和关键反应作为证据层后置展示**
- **数值预测必须尽量给出可验证数字，避免“建议按区间理解”这类占位废话**
